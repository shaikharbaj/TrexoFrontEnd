...
import { CreateUpdateCityModal } from "@/components/modals";
import { CityTable } from "@/components/tables";
...
const CityList: React.FunctionComponent<ICityProps> = () => {
  const t = useTranslations("CityPage");
  const { refresh } = useAppSelector(
    (state: RootState) => state.datatable
  );
  const handleOpenModal = async () => {
    await openPopup('city', `${t('Add')} ${t('City')}`, 'add');
  };
    ...
    <div className="space-y-6">
      <div className="flex items-center flex-wrap justify-between gap-4">
        <Breadcrumbs>
          ...
          <BreadcrumbItem>{t("City")}</BreadcrumbItem>
        </Breadcrumbs>
      </div>
      <div className="space-y-6">
        <div className="flex items-center flex-wrap justify-between gap-4">
      ...
      <div className="grid grid-cols-1 gap-6">
        <Card>
          <CardContent className="pt-6">
            <CityTable key={String(refresh)}  trans={t}/>
          </CardContent>
        </Card>
      </div>
    </div>
